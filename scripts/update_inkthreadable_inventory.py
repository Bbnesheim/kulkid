...
class ShopifyInventoryUpdater:
    def __init__(
        ...
        self._location_by_name: Dict[str, str] = {}
    ...
    def run(self, product_query: Optional[str]) -> None:
        ...
        self._location_by_name = location_by_name

        target_location_id = location_by_name.get(self.target_location)
        if target_location_id is None:
            fallback_location = _select_default_location(locations)
            if fallback_location:
                self.logger.warning(
                    "Target location '%s' not found; defaulting to '%s'",
                    self.target_location,
                    fallback_location["name"],
                )
                self.target_location = fallback_location["name"]
                target_location_id = str(fallback_location["id"])
            else:
                raise RuntimeError(
    ...
    def _iter_inkthreadable_variants(
        ...
        logging.info(f"Using product query: {query}")
...
def _resolve_location_id(
    location_node: Dict[str, object], location_by_name: Dict[str, str]
) -> str:
    location_gid = location_node.get("id")
    if location_gid:
        return gid_to_id(str(location_gid))
    location_name = location_node.get("name")
    if location_name and location_name in location_by_name:
        return location_by_name[location_name]
    return ""


def _select_default_location(locations: Sequence[Dict[str, object]]) -> Optional[Dict[str, object]]:
    for location in locations:
        if not location.get("legacy", False):
            return location
    for location in locations:
